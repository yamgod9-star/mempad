...
const Feed = {
  ...
  toggleLike(soundId, value) {
    ...
    if (prev === 1) sound.likes -= 1;
    if (prev === -1) sound.dislikes -= 1;

    const next = prev === value ? 0 : value;
    likes[soundId][user.id] = next;
    if (next === 1) sound.likes += 1;
    if (next === -1) sound.dislikes += 1;
  ...
  async deleteSound(soundId) {
    ...
    try { if (sound && sound._audioUrl && sound._audioUrl.startsWith && sound._audioUrl.startsWith('blob:')) URL.revokeObjectURL(sound._audioUrl); } catch (e) {}
  ...
  async postComment(text) {
    ...
    try {
      ...
      // Update UI
      await Feed.renderComments(soundId);
      // Update local sound comments count (best-effort)
      sound.commentsCount = (sound.commentsCount || 0) + 1;
      Feed.persist();
      Feed.refreshCardInPlace(soundId);
  ...
  async publishSound({ title, tags }) {
    ...
    try {
      ...
      const message = err?.message || 'Ошибка загрузки звука';
  ...
  openEdit(soundId) {
    ...
    document.querySelector('#upload-form h2')?.remove();
  ...
  bindUploadForm() {
    ...
    document.getElementById('upload-form').addEventListener('submit', (e) => {
      ...
      if (SupabaseAPI.storageAvailable === false && !form.dataset.editing) {
        UI.toast('Загрузка звуков отключена — хранилище Supabase недоступно.', 'error');
        return;
      }
      ...
      if (editingId) {
        ...
        const sound = Feed.publishSound({ title, tags });
        if (sound) {
