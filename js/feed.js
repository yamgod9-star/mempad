...
const Feed = {
  ...
  /* ---------- Загрузка / сохранение ---------- */
  async load() {
    // Load sounds from Supabase or fallback to local cache.
    try {
      const remoteSounds = await SupabaseAPI.getSounds();
      if (Array.isArray(remoteSounds) && remoteSounds.length) {
        Feed.sounds = remoteSounds;
      } else {
        const localSounds = Storage.get(STORAGE_KEYS.SOUNDS, []);
        Feed.sounds = localSounds.length ? localSounds : remoteSounds;
      }
    } catch (err) {
