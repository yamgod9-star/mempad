...
const STORAGE_KEYS = {
  ...
  REPORTS: 'mempad_reports',       // [report, ...]
  PLAYED_SOUNDS: 'mempad_played_sounds', // { [userId | anon]: [soundId, ...] }
...
function seedDatabaseIfNeeded() {
  ...
  const existingSounds = Storage.get(STORAGE_KEYS.SOUNDS, []);

  if (!hasSeeded) {
    if (!existingSounds.length) {
      const seedSounds = buildSeedSounds();
      Storage.set(STORAGE_KEYS.SOUNDS, seedSounds);
      Storage.set(STORAGE_KEYS.COMMENTS, buildSeedComments(seedSounds));
    }
  ...
  // Remove any previously seeded/demo sounds while preserving user data.
  const nonSeeded = existingSounds.filter(s => !seededAuthorIds.includes(s.authorId));
  if (nonSeeded.length !== existingSounds.length) {
  ...
  Object.keys(comments).forEach(k => {
    const sound = existingSounds.find(ss => ss.id === k);
    if (sound && seededAuthorIds.includes(sound.authorId)) { delete comments[k]; commentsChanged = true; }
  });
  ...
  Object.keys(likes).forEach(k => {
    const sound = existingSounds.find(ss => ss.id === k);
    if (sound && seededAuthorIds.includes(sound.authorId)) { delete likes[k]; likesChanged = true; }
  });
  ...
  Object.keys(favs).forEach(uid => {
    ...
    const after = before.filter(sid => {
      const sound = existingSounds.find(ss => ss.id === sid);
      return !(sound && seededAuthorIds.includes(sound.authorId));
    });
