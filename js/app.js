...
const App = {
  ...
  async init() {
    ...
    if (SupabaseAPI.storageAvailable === false) {
      UI.toast('Хранилище Supabase недоступно или bucket не найден. Загрузка звуков отключена.', 'error');
      document.querySelectorAll('#hero-upload, #upload-btn').forEach(btn => {
        if (btn instanceof HTMLButtonElement) {
          btn.disabled = true;
          btn.title = 'Загрузка звуков отключена: хранилище Supabase недоступно.';
        }
      });
    }
