...
const App = {
  ...
  navigate(route) {
    const isSoundAnchor = typeof route === 'string' && route.startsWith('sound-');
    if (!['feed', 'profile', 'admin'].includes(route) && !isSoundAnchor) route = 'feed';

    const shouldStopPlayback = !isSoundAnchor && App.currentRoute !== route;
    App.currentRoute = isSoundAnchor ? 'feed' : route;
    ...
    const pageEl = document.getElementById(`page-${App.currentRoute}`);
    pageEl.classList.remove('hidden');

    document.querySelectorAll('.nav-link').forEach(l => l.classList.toggle('active', l.dataset.route === App.currentRoute));
    ...
    if (isSoundAnchor) {
      const soundId = route.replace(/^sound-/, '');
      Feed.renderHighlightedSound(soundId);
      return;
    }
