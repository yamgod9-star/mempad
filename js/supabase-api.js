...
const SupabaseAPI = {
  ...
  async deleteComment(commentId) {
    try {
      ...
      if (!commentsAvailable) {
        const commentsMap = Storage.get(STORAGE_KEYS.COMMENTS, {});
        Object.keys(commentsMap).forEach((sid) => {
          const previousLength = Array.isArray(commentsMap[sid]) ? commentsMap[sid].length : 0;
          commentsMap[sid] = (commentsMap[sid] || []).filter(c => c.id !== commentId);
          if (soundId === null && commentsMap[sid].length !== previousLength) {
            soundId = sid;
          }
        });
        Storage.set(STORAGE_KEYS.COMMENTS, commentsMap);
      } else {
        const { data: comment, error: fetchError } = await supabaseClient.from('comments').select('sound_id').eq('id', commentId).single();
        if (fetchError) {
          const msg = String(fetchError.message || '').toLowerCase();
          if (fetchError.status === 404 || /relation.*comments/i.test(msg) || /could not find the table/.test(msg)) {
            const commentsMap = Storage.get(STORAGE_KEYS.COMMENTS, {});
            Object.keys(commentsMap).forEach((sid) => {
              commentsMap[sid] = (commentsMap[sid] || []).filter(c => c.id !== commentId);
            });
        ...
        soundId = comment && comment.sound_id ? comment.sound_id : null;
      ...
      if (soundId) {
        try {
          ...
          console.warn('Failed to decrement comment count after deletion:', counterErr);
        }

        if (window.Feed && typeof Feed.getSoundById === 'function') {
          const sound = Feed.getSoundById(soundId);
          if (sound) {
            sound.commentsCount = Math.max(0, (sound.commentsCount || 0) - 1);
            if (typeof Feed.persist === 'function') {
              Feed.persist();
            }
          }
        }
